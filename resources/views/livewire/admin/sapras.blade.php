        ...
        <div class="container-xl">
            <div class="row g-2 align-items-center">
                ...
                <div class="col-auto ms-auto d-print-none">
                    <div class="btn-list">
                        <a href="/panel-admin/sarana-prasarana/add-sarana-prasarana" wire:navigate
                ...
                <div class="table-responsive">
                    <div class="table-responsive">
                        <table class="table table-vcenter card-table">
                                ...
                                <tr>
                                    <th class="w-1">
                                        ...
                                        <input type="hidden" wire:model.live="firstId"
                                            value="{{ $sapras && $sapras->isNotEmpty() ? $sapras->first()->id : 0 }}">
                                    </th>
                                    <th>No</th>
                                    <th>Name</th>
                                    <th>Condition</th>
                            ...
                            <tbody>
                                @if ($sapras->isNotEmpty())
                                    <?php $no = 1; ?>
                                    @foreach ($sapras as $s)
                                        <tr>
                                            <td>
                                                <input wire:key="{{ $s->id }}" value="{{ $s->id }}"
                                            ...
                                            <td class=" text-center">
                                                ...
                                                @else
                                                    @php
                                                        $photos = is_string($s->foto)
                                                            ? json_decode($s->foto, true)
                                                            : $s->foto;
                                                        $firstPhoto = $photos[0];
                                                    @endphp
                                                    <a data-fslightbox="sapras-{{ $s->id }}"
                                                        href="{{ asset('uploads/sapras/' . $firstPhoto['filename']) }}">
                                                        <div class="avatar">
                                                            <img class=" rounded rounded-2"
                                                                src="{{ asset('uploads/sapras/' . $firstPhoto['filename']) }}"
                                                                alt="" srcset="">
                                                        </div>
                                                    </a>
                                            ...
                                            <td>{{ $s->created_at }}</td>
                                            <td>{{ $s->updated_at ?? '----------' }}</td>
                                            <td wire:key="sapras-{{ $s->id }}">
                                                <div class="btn-list flex-nowrap">
                                                    <a wire:navigate
                                                        href="/panel-admin/sarana-prasarana/{{ $s->id }}/edit-sarana-prasarana"
                                                    ...
                                                    <button wire:click="delete({{ $s->id }})"
                                                        ...
                                                        <span wire:loading wire:target="delete({{ $s->id }})"
                                                            class="spinner-border spinner-border-sm" role="status"
                                                            aria-hidden="true"></span>
                                                        <svg wire:loading.remove
                                                            wire:target="delete({{ $s->id }})"
